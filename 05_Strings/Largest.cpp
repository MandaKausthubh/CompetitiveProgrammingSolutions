...
void solve() {
    ...
    // printvc(subset);
    int ans = sz(subset) - count(all(S), S[subset.front()]);
    // reverse(all(subset));
    forw(i, 0, sz(subset)-1) {if (i < sz(subset) - i - 1) swap(S[subset[i]], S[subset[sz(subset) - 1 - i]]);}
    bool val = true;
    forw(i, 1, n-1) val = val && (S[i-1] <= S[i]);
