...
void solve() {
    ...
    forw(i, 2, n) {
        // DPdc[i] = (DPdc[i-1] + ((i-k >= 0) ? DPdc[i-k] : 0)) % MOD - DPdc[i-2];
        DPdc[i] = 0;
        forw(j, 1, k) DPdc[i] = DPdc[i] + ((i-j>=0) ? DPdc[i-j]:0) % MOD;
    }
    ...
    forw(i,1,n) {
        if(i < d) DP[i] = 0;
            ...
            DP[i] = 0;
            forw(j,1,d-1) DP[i] = (DP[i] + ((i-j>=0) ? DP[i-j]:0))  % MOD;
            forw(j,d,k) DP[i] = (DP[i] + ((i-j>=0) ? DPdc[i-j]:0))  % MOD;
        }
    }

    // forw(i, 0, n) cout << DP[i] << ' ';
    // cout <<  '\n';
