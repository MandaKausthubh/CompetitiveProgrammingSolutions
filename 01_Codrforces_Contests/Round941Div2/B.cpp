...
void solve() {
    ...
    if(V[1][1] == V[1][m] && V[n][1] == V[n][m] && V[1][1] != V[n][m]) {
        forw(i,1,m) p |= (V[1][i] != V[1][1]);
        forw(i,1,m) p |= (V[n][i] != V[n][m]);
    }
    if(V[1][1] == V[n][1] && V[1][m] == V[n][m] && V[1][1] != V[n][m]) {
        forw(i,1,n) q |= (V[i][1] != V[1][1]);
        forw(i,1,n) q |= (V[i][m] != V[n][m]);
    }

    if( (a==d) || (b==c) || p || q) cout << "YES\n";
    else cout << "NO\n";
}
